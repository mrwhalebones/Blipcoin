...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("blipcoin"))
        return false;
...
{
    // Convert blipcoin:// to blipcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("blipcoin://"))
    {
        uri.replace(0, 12, "blipcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Blipcoin.lnk";
}
...
{
    return GetAutostartDir() / "blipcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Blipcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  blipcoin-qt [" + tr("command-line options") + "]                     " + "\n";
